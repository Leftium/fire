...
{
      if (x < 0 || x >= FIRE_W || y < 0 || y >= FIRE_H) return 0;
...
inline USINT fire_math(int x, int y, USINT *fire)
{
   ...
   // total+=get_fire(fire,(x+0), (y+5));
   
   // total+=get_fire(fire,(x+1), (y-6));
   // total+=get_fire(fire,(x+0), (y+6));
   // total+=get_fire(fire,(x-1), (y-6));
...
void add_hotspot(USINT *fire, int x, int y, int hotspot_radius, int num_spots)
{
   for (int j= 0; j<num_spots; j++)
   {
      ...
      if (get_fire(fire, hotspot_x, hotspot_y) < 160) set_fire(fire, hotspot_x, hotspot_y, 160);
      add_fire(fire, hotspot_x, hotspot_y, 4);
      if (get_fire(fire, hotspot_x, hotspot_y) > 255) set_fire(fire, hotspot_x, hotspot_y, 255);
